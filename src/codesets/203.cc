// 203 - European Supplementary Latin Set ("Latin 9")
...
void
iso_8859_15::decode (unsigned char c, std::u16string &out)
{
  char16_t ch = c + 0x80;
...
void
iso_8859_15::invoke (code_element elt, std::string &out)
{
  switch (elt) {
  case ELEMENT_G0:
    throw std::runtime_error("Attempted to invoke ISO-8859-15 in G0");
  case ELEMENT_G1:
    out += "\x1b\x2d\x62";
    break;
  case ELEMENT_G2:
...
bool
iso_8859_15::encode (const char16_t *&pcs,
                     ...
                     std::string &out)
{
  if (pcsend == pcs + 1) {
    ...
    if (*pcs >= 0xa0 && *pcs <= 0xff) {
      ...
      switch (*pcs) {
      case 0x0178: chout = 0xbe; break;
      case 0x0160: chout = 0xa6; break;
      case 0x0161: chout = 0xa7; break;
      case 0x017d: chout = 0xb4; break;
      case 0x017e: chout = 0xb8; break;
      case 0x0152: chout = 0xbc; break;
      case 0x0153: chout = 0xbd; break;
      case 0x20ac: chout = 0xa4; break;
      default:
        return false;
      }
    }
  ...
  case 'Y': case 'y':
    ...
    if (ch == 'Y' && *pcs == 0x0308) { chout = (char)0xbe; break; }
    if (ch == 'y' && *pcs == 0x0308) { chout = (char)0xff; break; }
    return false;
  case 'S':
    if (*pcs == 0x030c) { chout = (char)0xa6; break; }
    return false;
  case 's':
    if (*pcs == 0x030c) { chout = (char)0xa8; break; }
    return false;
  case 'Z':
    if (*pcs == 0x030c) { chout = (char)0xb4; break; }
    return false;
  case 'z':
    if (*pcs == 0x030c) { chout = (char)0xb8; break; }
    return false;
  default:
    return false;
  }
